...
//	HWND hWnd;	
	...
	CHustBaseDoc *pTreeDoc;
	pTreeDoc = CHustBaseDoc::GetDoc();
	CHustBaseApp::pathvalue = true;
	pTreeDoc->m_pTreeView->PopulateTree();
	pPaneShow->SetWindowText("");
}
...
int CEditArea::iReadDictstruct(char tabname[][20],int *tabnum,char colname[][20][20],int colnum[],AttrType coltype[][20],int collength[][20],int coloffset[][20],int iscolindex[][20])//编写新的传递系统表和系统列信息的函数
{
	...
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to open system table file.");
	rc=RM_OpenFile((LPSTR)(LPCTSTR)column,&colfilehandle);//去SYSCOLUMNS表中获取列名
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to open system columns file.");
	rc=OpenScan(&FileScan1,&fileHandle,0,NULL);
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to initialize scanning file.");
	while(GetNextRec(&FileScan1,&rec1)==SUCCESS)
	{
		...
		if(rc!=SUCCESS)
			AfxMessageBox("Fail to initialize scanning column file.");
	...
	*tabnum=i;
	...
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to close system column file.");
	return 1;
}
