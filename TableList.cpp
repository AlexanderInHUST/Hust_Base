...
void CTableList::displayTabInfo(CString ParentNode)
{
	...
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to open system table.");
	condition.bLhsIsAttr=1;
	condition.bRhsIsAttr=0;
	condition.LattrOffset=0;
	condition.LattrLength = ParentNode.GetLength() + 1;
	...
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to initialize scanning files.");
	rc=GetNextRec(&FileScan,&rec);
	if(rc!=SUCCESS)//点击的不是表名，不做显示处理
	{
		rc=RM_CloseFile(&fileHandle);
		if(rc!=SUCCESS)
			AfxMessageBox("Fail to close system table file.");
		return;
	}
	rc=RM_CloseFile(&fileHandle);
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to close system table files.");
...
// 	CTreeCtrl & ctc = GetDocument()->m_pTreeView->GetTreeCtrl();
	...
	strcpy(table_name, sys_dbname);
	strcat(table_name, ".tb.");
	strcat(table_name, ParentNode);
	rc=RM_OpenFile(table_name, &fileHandle);
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to open table file.");
	FileScan.bOpen=0;
	...
	rc=RM_CloseFile(&fileHandle);
	if(rc!=SUCCESS)
		AfxMessageBox("Fail to close table file.");
}