...
RC CreateIndex(const char *fileName, AttrType attrType, int attrLength) {
	...
	ix_fileHeader->order = keys_size - 2 < 2 ? 2 : keys_size - 2; // debug here!
...
bool checkConditions(IX_IndexScan *indexScan, char * check_value) {
	...
	auto comparedKey = new char[keyLength];
...
RC InsertEntry(IX_IndexHandle *indexHandle, char *pData, const RID *rid) {
	...
	auto realKey = new char[keyLength];
...
//
		...
		for (int i = 0; i < newIdxPos; i++) {
			...
			auto addedKey = new char[keyLength];
...
RC DeleteEntry(IX_IndexHandle *indexHandle, char *pData, const RID *rid) {
	...
	auto dumpKey = new char[keyLength];
	auto dumpRid = new char[sizeof(RID)];
	auto dumpPageNum = new char[sizeof(PageNum)];
	...
	while (!isValid) {
		...
		if (isLeftEnough) {
			if (aimNode->is_leaf) {
				...
				auto borrowedKey = new char[keyLength];
				auto borrowedRid = new char[sizeof(RID)];
				...
				auto borrowedKey = new char[keyLength];
				auto newParentKey = new char[keyLength];
			...
			if (aimNode->is_leaf) {
				...
				auto newKey = new char[keyLength];
			...
			if (aimNode == nullptr) {
				...
				auto downParentKey = new char[keyLength];
